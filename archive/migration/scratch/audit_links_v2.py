...
def slugify(text):
    ...
    text = text.replace(' ', '-')
    # Remove some common symbols but keep Sanskrit/accented
...
def process_licenses(licenses_path, lektionen_dir):
    ...
    for line in lines:
        ...
        if match:
            ...
            is_general = "#_lektion-" in current_url or current_url.endswith(f"lektion{lesson_num}")
...
if __name__ == "__main__":
    ...
    with open("/Volumes/SanDisk1TB/proj/Payer/scratch/licenses_updated_v2.md", "w", encoding="utf-8") as f:
        f.write(new_content)
