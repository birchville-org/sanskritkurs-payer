...
def restore_integrity(de_path, bg_path):
    ...
    for i in range(len(bg_lines)):
        if i not in bg_to_de:
            best_a = -1
            best_score = 0
            for a in range(max(0, i-15), min(len(de_lines), i+15)):
    ...
    for i, bg_l in enumerate(bg_lines):
        if i in bg_to_de:
            ...
            # If German line is mostly Sanskrit, or starts with Sanskrit punctuation, just take it.
            if de_ratio > 0.4 or de_l.strip().startswith(">") or de_l.strip().startswith("|"):
                # But wait, if it's a table header with Bulgarian text, we should be careful.
                # Actually, German table headers are also often Sanskrit or just | | |.
                new_bg_lines.append(de_l)
            elif re.search(r'[अ-ह][\u0400-\u04FF]|[\u0400-\u04FF][अ-ह]', bg_l):
                # If mixed script, and German has Sanskrit, overwrite.
                if de_ratio > 0:
                    new_bg_lines.append(de_l)
                else:
                    new_bg_lines.append(bg_l)
            else:
                # Token-level for low-Sanskrit lines
                bg_toks = re.split(r'(\s+|[|,.()!?;:\"\'\[\]\-\*])', bg_l)
                de_toks = re.split(r'(\s+|[|,.()!?;:\"\'\[\]\-\*])', de_l)
                de_s_toks = [t for t in de_toks if is_sanskrit_token(t)]
                
                # If the line looks like a phoneticization (many Cyrillic tokens where DE has Sanskrit)
                bg_s_like_toks = [t for t in bg_toks if is_sanskrit_token(t) or (len(t) > 2 and re.search(r'[\u0400-\u04FF]', t))]
                if len(de_s_toks) > 0 and len(bg_s_like_toks) >= len(de_s_toks):
                    # Replace sequentially and discard extra "Sanskrit-like" tokens in BG
                    dt_ptr = 0
                    new_toks = []
                    for bt in bg_toks:
                        if bt in bg_s_like_toks:
                            if dt_ptr < len(de_s_toks):
                                new_toks.append(de_s_toks[dt_ptr])
                                dt_ptr += 1
                            else:
                                pass # Discard extra phonetic tokens
                        else:
                            new_toks.append(bt)
