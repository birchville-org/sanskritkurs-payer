...
The Veda (śruti) and the tradition (smṛti) state that one cannot hear anything unjust from a master.
...
(Uppruni myndar: [Upplýsingar](/is/licenses#uebung040))
...
From which one withdraws, from that one is freed. For if one withdraws from everything, then one knows no suffering, however small it may be like an atom.
...
(Uppruni myndar: [Upplýsingar](/is/licenses#uebung041))
...
(Uppruni myndar: [Upplýsingar](/is/licenses#uebung042))
