---
title: "Lección 4"
subtitle: "Pronombres interrogativos y pronombres demostrativos en nominativo"
lesson_id: 4
last_reconstructed: 2026-05-07
category: "Grammatik"
status: stable

---

# Lección 4

## 4.1. Preguntas de complemento simples (preguntas por palabra) y respuestas

### 4.1.1. Pronombres interrogativos y pronombres demostrativos

#### 4.1.1.1. Pronombres interrogativos (praśnārthakasarvanāma n. = ⟪प्रश्नार्थकसर्वनामन्⟫)

::: grammar-box
| | Nominativo singular | Nominativo plural |
| :--- | :--- | :--- |
| **Neutro** | **kim = ⟪किम्⟫** | **kāni = ⟪कानि⟫** |
| **Masculino** | **kas = ⟪कस्⟫** | **ke = ⟪के⟫** |
| **Femenino** | **kā = ⟪का⟫** | **kās = ⟪कास्⟫** |
:::
#### 4.1.1.2. Pronombres demostrativos (nidarśakasarvanāmāni n. = ⟪निदर्शकसर्वनामानि⟫)

::: grammar-box
| **Raíces** ||||
| :--- | :---: | :---: | :---: |
| | **tad = ⟪तद्⟫:br"él, ellos, ello; el, la, lo":br(Mencionado)** | **etad = ⟪एतद्⟫:br"este, esta, esto":br(Muy cerca del hablante)** | **idam = ⟪इदम्⟫:br"este, esta, esto":br(Cerca)** |
| **Nominativo singular** ||||
| **Neutro** | **tad = ⟪तद्⟫** | **etad = ⟪एतद्⟫** | **idam = ⟪इदम्⟫** |
| **Masculino** | **sa, so saḥ = ⟪स सो सः⟫** | **eṣa, eṣo, eṣaḥ = ⟪एष एषो एषः⟫** | **ayam = ⟪अयम्⟫** |
| **Femenino** | **sā = ⟪सा⟫** | **eṣā = ⟪एषा⟫** | **iyam = ⟪इयम्⟫** |
| **Nominativo plural** ||||
| **Neutro** | **tāni = ⟪तानि⟫** | **etāni = ⟪एतानि⟫** | **imāni = ⟪इमानि⟫** |
| **Masculino** | **te = ⟪ते⟫** | **ete = ⟪एते⟫** | **ime = ⟪इमे⟫** |
| **Femenino** | **tās = ⟪तास्⟫** | **etās = ⟪एतास्⟫** | **imās ⟪इमास्⟫** |
:::
**Para el Nom. sg. masc.:**
**sa, eṣa** (⟪स एष⟫) se coloca antes de todos los sonidos excepto la **-a** corta
**so, eṣo** (⟪सो एषो⟫) se coloca antes de **-a**, que se elide con Avagraha (desaparece)
**saḥ, eṣaḥ** (⟪सः एषः⟫) se coloca solo en pausa (final de oración, final de verso)

Para poder formar oraciones con estos pronombres (sarvanāman n.), se necesita además conocer las siguientes reglas de Sandhi:

### 4.1.2. Sandhi de -m final

::: grammar-box
**El -m final se reemplaza por Anusvāra (-ṃ) antes de consonantes. Al final de oración o verso, y antes de vocales, se conserva el -m.**
:::
### 4.1.3. Sandhi de -e final

::: grammar-box
1. **ante a- permanece -e sin cambiar: el a- inicial se elide con Avagraha:**
   p. ej. ete + agnayas » ete 'gnayaḥ = ⟪एते⟫ ⟪ऽग्नयः⟫ "Estos fuegos"

2. **Ante otras vocales que no sea a- breve, -e se reemplaza habitualmente por -a con hiato. También se produce el reemplazo por -ay:**
   p. ej. ime + īśvarās » ima īśvarāḥ = ⟪इम⟫ ⟪ईश्वराः⟫ (raro: imay īśvarāh = ⟪इमयीश्वराः⟫) "Estos SEÑORES"
:::
### 4.1.4. Sandhi de -d final

::: grammar-box
**El -d final se reemplaza por -t en pausa, así como ante guturales sordos (k, kh) y labiales (p, ph).**

El reemplazo ante otros consonantes se tratará más adelante. ¡Evite por el momento dichas colisiones fonéticas!
:::
### 4.1.5. Fórmulas interrogativas

Mediante estos pronombres se forman, por ejemplo, las siguientes fórmulas interrogativas:

::: no-header
| | | | |
| :--- | :--- | :--- | :--- |
| **Singular** | **Neutro** | tat kim? /kiṃ tat?:br⟪तत्किम्⟫, ⟪किं⟫ ⟪तत्⟫:bretat kim? /kim etat?:br⟪एतत्किम्⟫, ⟪किमेतत्⟫:bridaṃ kim? / kim idam:br⟪इदं⟫ ⟪किम्⟫, ⟪किमिदम्⟫ | «¿Qué es eso?» |
| ^^ | **Masculino** | sa kaḥ? / kaḥ saḥ?:br⟪स⟫ ⟪कः⟫, ⟪कः⟫ ⟪सः⟫:breṣa kaḥ? / ka eṣaḥ:br⟪एष⟫ ⟪कः⟫, ⟪क⟫ ⟪एषः⟫:brayaṃ kaḥ? / ko 'yam?:br⟪अयं⟫ ⟪कः⟫, ⟪को⟫ ⟪ऽयम्⟫ | «¿Quién es eso?»:br«¿Qué es el?» |
| ^^ | **Femenino** | sā kā? / kā sā?:br⟪सा⟫ ⟪का⟫, ⟪का⟫ ⟪सा⟫:breṣā kā? / kaiṣā? (= kā + eṣā):br⟪एषा⟫ ⟪का⟫, ⟪कैषा⟫:briyaṃ kā? / keyam? (= kā + iyam):br⟪इयं⟫ ⟪का⟫, ⟪केयम्⟫ | «¿Quién es eso?»:br«¿Quién es ella?» |
| **Plural** | **Neutro** | tāni kāni? / kāni tāni?:br⟪तानि⟫ ⟪कानि⟫, ⟪कानि⟫ ⟪तानि⟫:bretāni kāni? / kāny etāni?:br⟪एतानि⟫ ⟪कानि⟫, ⟪कान्येतानि⟫:brimāni kāni? / kānīmāni? (= kāni + imāni):br⟪इमानि⟫ ⟪कानि⟫, ⟪कानीमानि⟫ | |
| ^^ | **Masculino** | te ke? / ke te?:br⟪ते⟫ ⟪के⟫, ⟪के⟫ ⟪ते⟫:brete ke? / ka ete?:br⟪एते⟫ ⟪के⟫, ⟪क⟫ ⟪एते⟫:brime ke? / ka ime?:br⟪इमे⟫ ⟪के⟫, ⟪क⟫ ⟪इमे⟫ | |
| ^^ | **Femenino** | tāḥ kāḥ? / kās tāḥ?:br⟪ताः⟫ ⟪काः⟫, ⟪कास्ताः⟫:bretāḥ kāḥ? / kā etāḥ?:br⟪एताः⟫ ⟪काः⟫, ⟪का⟫ ⟪एताः⟫:brimāḥ kāḥ? / kā imāḥ?:br⟪इमाः⟫ ⟪काः⟫, ⟪का⟫ ⟪इमाः⟫ | |
:::
Un ejemplo de otras preguntas:

> viṣṇuḥ kaḥ? = ⟪विष्णुः⟫ ⟪कः⟫ Respuesta: viṣṇur īśvaraḥ = ⟪विष्णुरीश्वरः⟫. (Aquí se puede colocar el sujeto al principio de la oración nominal, por ejemplo, para enfatizar la conexión con la pregunta.)

NOTA: Las respuestas que puede dar a estas preguntas con sus conocimientos actuales de sánscrito, por supuesto, aún no son idiomáticamente perfectas en todos los aspectos.

## 4.2. Ejercicio

**A) Forme oralmente preguntas con las siguientes palabras según el esquema viṣṇuḥ kaḥ (⟪विष्णुः⟫ ⟪कः⟫) y responda las preguntas en sánscrito:**

śruti, śiva, brāhmaṇa, dvija (plural), indrāṇī, dhenu, tulādhara, kālidāsa

= ⟪श्रुति⟫, ⟪शिव⟫, ⟪ब्राह्मण⟫, ⟪द्विज⟫ (⟪बहुवचनम्⟫), ⟪इन्द्राणी⟫, ⟪धेनु⟫, ⟪तुलाधर⟫, ⟪कालिदास⟫

**B) Forme preguntas para el siguiente ejercicio de lectura según el patrón etat kim (⟪एतत्किम्⟫) y responda las preguntas con las palabras indicadas y los pronombres demostrativos, por ejemplo eṣa bālaḥ (⟪एष⟫ ⟪बालः⟫):**

::: media
![](/images/lekt0401.webp)
(Fuente de la imagen: [Detalles](/es/licenses#lekt0401))
:::
 \<!--  --\>