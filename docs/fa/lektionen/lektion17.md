---
title: "درس ۱۷"
subtitle: "کلاس دوم زمان حال (⟪अदादि⟫ = ⟪अद्⟫ و غیره)"
lesson_id: 17
last_reconstructed: 2026-05-21
category: "Grammatik"
status: stable

---

# درس ۱۷

## ۱۷.۱. کلاس دوم فعل حال (⟪अदादि⟫ = ⟪अद्⟫ و غیره)

پسوندهای فعلی مستقیماً به ریشه می‌پیوندند. در این مورد باید تغییرات آوایی ناشی از ساندهای واژگانی (Wortsandhi) مورد توجه قرار گیرد.

::: grammar-box

اشکال ساختاری زیر در کلاس دوم فعل حال وجود دارند:

۱. ریشه‌هایی با تغییر سطح بن (تغییر درجه بن)
*   بیشتر: سطح بالا - سطح پایین
*   برخی ریشه‌ها: سطح دراز (درجه کشیده) - سطح پایین
۲. ریشه‌هایی بدون تغییر سطح بن
۳. ریشه‌های دو هجایی با پایان -i/-ī همراه با تغییر سطح بن

:::

## ۱۷.۲. ریشه‌های کلاس دوم فعل حال با تغییر سطح بن: سطح بالا - سطح پایین

نمونه‌ها:

⟪द्विष्⟫ ۲ U "نفرت داشتن"

*   بن قوی حال: dveṣ-
*   بن ضعیف حال: dviṣ-

::: no-header

| | |
| :--- | :--- |
| ۳\. مفرد حال P | ⟪द्वेष्टि⟫ (« dveṣ + -ti) |
| ۳\. جمع حال P | ⟪द्विषन्ति⟫ (dviṣ-anti) |
| ۳\. مفرد حال Ā | ⟪द्विष्टे⟫ (« dviṣ + -te) |
| ۳\. جمع حال Ā | ⟪द्विषते⟫ (dviṣ-ate) |

:::

در اینجا قانون ساندهای واژگانی زیر اعمال می‌شود:

::: grammar-box

ساندهای واژگانی از -ṣ + t(h)-

\-ṣ + t(h)- » -ṣṭ(h)-

:::

نمونه‌های بیشتر:

⟪इ⟫ ۲ P "رفتن" (ġا پس از برخی پیشوندهای فعلی)

*   بن قوی حال: e-
*   بن ضعیف حال:
*   پیش از همخوان: i-
*   پیش از مصوت: y-/iy-

::: no-header

| | |
| :--- | :--- |
| ۳\. مفرد حال P | ⟪एति⟫ (e-ti) |
| ۳\. جمع حال P | ⟪यन्ति⟫ (y-anti) |
| ۳\. مفرد حال Ā | ⟪इते⟫ (i-te) |
| ۳\. جمع حال Ā | ⟪इयते⟫ (iy-ate) |

:::

⟪दुह्⟫ ۲ U "دوشیدن"

*   بن قوی حال: doh-
*   بن ضعیف حال: duh-

::: no-header

| | |
| :--- | :--- |
| ۳\. مفرد حال P | ⟪दोग्धि⟫ (« doh- + -ti) |
| ۳\. جمع حال P | ⟪दुहन्ति⟫ (duh-anti) |
| ۳\. مفرد حال Ā | ⟪दुग्धे⟫ (« duh- + -te) |
| ۳\. جمع حال Ā | ⟪दुहते⟫ (duh-ate) |

:::

⟪हन्⟫ ۲ P "کشتن، ضرب زدن، قتل کردن"

*   ریشه مضارع قوی: han-
*   ریشه مضارع ضعیف:
    *   پیش از همخوان: ha- (« \*hn-)
    *   پیش از مصوت: ghn-/han-

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪हन्ति⟫ (han-ti) |
| 3\. pl. Präs. P | ⟪घ्नन्ति⟫ (ghn-anti) |
| 3\. sg. Präs. Ā | ⟪हते⟫ (ha-te) |
| 3\. pl. Präs. Ā | ⟪घ्नते⟫ (ghn-ate) |

:::

⟪अस्⟫ 2 P "بودن"

*   بنای قوی حال: as-
*   بنای ضعیف حال: s-

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪अस्ति⟫ (as-ti) |
| 3\. pl. Präs. P | ⟪सन्ति⟫ (s-anti) |

:::

## 17.3. ریشه‌های طبقه دوم زمان حال با درجه‌بندی ستاک: درجه کشیده - درجه کوتاه

مثال:

⟪स्तु⟫ ۲ U "ستودن"

*   پایه مضارع قوی: stau-
*   پایه مضارع ضعیف:
    *   پیش از همخوان: stu-
    *   پیش از واکه: stuv-

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪स्तौति⟫ (stau-ti):broder: ⟪स्तवीति⟫ |
| 3\. pl. Präs. P | ⟪स्तुवन्ति⟫ (stuv-anti) |
| 3\. sg. Präs. Ā | ⟪स्तुते⟫ (stu-te) |
| 3\. pl. Präs. Ā | ⟪स्तुवते⟫ (stuv-ate) |

:::

## 17.4. ریشه‌های طبقه دوم زمان حال بدون درجه‌بندی ستاک

::: grammar-box

بدون طبقه‌بندی ریشه، بن‌های کلاس دوم فعل مضارع بر -ā

:::

مثال:

⟪पा⟫  شخص دوم «حفاظت کردن، نگه داشتن»

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪पाति⟫ (pā-ti) |
| 3\. pl. Präs. P | ⟪पान्ति⟫ (« pā- + -anti) |

:::

همچنین برخی از ریشه‌های دیگر دسته دوم نیز فاقد تغییر پایه هستند:

مثال‌ها:

⟪अद्⟫ ۲ P "خوردن"

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪अत्ति⟫ (« ad- + -ti) |
| 3\. pl. Präs. P | ⟪अदन्ति⟫ (ad-anti) |

:::

⟪आस्⟫ ۲ Ā "نشستن"

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. Ā | ⟪आस्ते⟫ (ās-te) |
| 3\. pl. Präs. Ā | ⟪आसते⟫ (ās-ate) |

:::

⟪वच्⟫ 2 P «گفتن»

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪वक्ति⟫ (« vac- + -ti) |
| 3\. pl. Präs. P | kommt nicht vor |

:::

## 17.5. ریشه‌های دو هجایی بر -i/-ī با تغییر درجه ریشه

چندین ریشه در سلسله‌ای از شکل‌ها دوسه‌ای هستند، یعنی آن‌ها پیش از پایان همخوانی دارای -i (و در برابر برخی پایان‌ها -ī) هستند. با این حال، این ریشه‌ها توسط دستوران بومی و در واژه‌نامه‌های غیره به عنوان یک‌بخشی در نظر گرفته می‌شوند.

مثال‌ها:

⟪रुद्⟫ 2 P "گریستن، ناله کردن"

*   پایه قوی حال: rodi-
*   پایه ضعیف حال:
    *   پیش از همخوانی: rudi-
    *   پیش از مصوت: rud-

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪रोदिति⟫ (rodi-ti) |
| 3\. pl. Präs. P | ⟪रुदन्ति⟫ (rud-anti) |

:::

⟪ब्रू⟫ 2 U «گفتن»

*   ریشه مضارع قوی: bravī- (« bro- + -ī)
*   ریشه مضارع ضعیف:
    *   پیش از همخوان: brū-
    *   پیش از واکه: bruv-

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪ब्रवीति⟫ (bravī-ti) |
| 3\. pl. Präs. P | ⟪ब्रुवन्ति⟫ (bruv-anti) |
| 3\. sg. Präs. Ā | ⟪ब्रूते⟫ (brū-te) |
| 3\. pl. Präs. Ā | ⟪ब्रुवते⟫ (bruv-ate) |

:::

همچنین ⟪स्तु⟫ 2 U «ستایش می‌کنند» علاوه بر اشکال ذکر شده در بخش ۳، اشکالی با این الگو دارد:

::: no-header

| | |
| :--- | :--- |
| 3\. sg. Präs. P | ⟪स्तवीति⟫ (« sto + ī + ti):broder: ⟪स्तौति⟫ |

:::

## 17.6. فهرست واژگان

⟪हन्⟫ ۲ پ ⟪हन्ति⟫، ⟪घ्नन्ति⟫ مفعولی ⟪हन्यते⟫  پ‌پ‌م ⟪हत⟫ : کشتن، از بین بردن، قتل

از آن:

::: indent

⟪घात⟫ م.: کشتن

:::

::: media

![](/images/lekt1703.webp)

تصویر: ⟪घाताः⟫
بنگالورو = ಬೆಂಗಳೂರು
(منبع تصویر: [جزئیات](/fa/licenses#lekt1703))

:::

⟪आस्⟫ ۲آ ⟪आस्ते⟫ مفعولی ⟪आस्यते⟫  پ‌پ‌م ⟪आसित⟫ : نشستن

از آن:

::: indent

⟪आसन⟫ n.: نشستن، جایگاه؛ همچنین: وضعیت‌های نشستن یوگی

:::

::: media

![](/images/lekt1701.webp)

تصویر: ⟪योगासनम्⟫
(منبع تصویر: [جزئیات](/fa/licenses#lekt1701))

:::

⟪रुद्⟫ 2 P ⟪रोदिति⟫ Pass. ⟪रुद्यते⟫ PPP ⟪रुदित⟫ : گریستن، ناله کردن

از آن:

::: indent

⟪रुद्र⟫ مذکر: (به معنای غرش‌کننده =) خدای طوفان رودرا

:::

⟪ब्रू⟫ ۲ مفرد ⟪ब्रवीति⟫ آ ⟪ब्रूते⟫ بدون مجهول و پارتیسیپ گذشته: صحبت کردن، گفتن (چیزی به کسی: دو مورد مفعولی مستقیم)

⟪दुह्⟫ ۲ مفرد ⟪दोग्धि⟫ مجهول ⟪दुह्यते⟫ پارتیسیپ گذشته ⟪दुग्ध⟫ : شیر گرفتن

::: media

![](/images/lekt1702.webp)

تصویر: ⟪दोग्धि⟫
(منبع تصویر: [جزئیات](/fa/licenses#lekt1702))

:::

⟪दिश्⟫ ۶ مفرد ⟪दिशति⟫ مجهول ⟪दिश्यते⟫ پارتیسیپ گذشته ⟪दिष्ट⟫ : نشان دادن، دستور دادن، امر کردن

از آن:

::: indent

⟪दिष्टि⟫ مؤنث: دستور، تقدیر خوش

⟪दिष्ट्या⟫ حالت ابزاری: (تحت‌اللفظی: به واسطهٔ تقدیری خوش) ای تقدیر خوش (ندای شادی و خوشبختی)

:::

## 17.7. تمرین 1

الف) فعل را در جملات زیر قرار دهید و ترجمه کنید:

⟪१⟫. ⟪ब्राह्मणो⟫ ⟪ऽनृतं⟫ ⟪न⟫ ... (⟪ब्रू⟫ ⟪।⟫ ⟪वच्⟫ ⟪।⟫ ⟪वद्⟫)

⟪२⟫. ⟪क्षत्रियो⟫ ⟪जनान्⟫ ... (⟪पा⟫ ⟪।⟫ ⟪रक्ष्⟫)

⟪३⟫. ⟪बलवद्योधो⟫ ⟪द्विजारीन्⟫ ... (⟪जि⟫ ⟪।⟫ ⟪हन्⟫ ⟪।⟫ ⟪युध्⟫)

⟪४⟫. ⟪ब्राह्मणकविर्लोकेश्वरम्⟫ ... (⟪स्तु⟫ ⟪।⟫ ⟪यज्⟫)

⟪५⟫. ⟪अग्निर्यज्ञान्नम्⟫ ... (⟪अद्⟫ ⟪।⟫ ⟪दह्⟫)

⟪६⟫. ⟪बालवैश्यो⟫ ⟪धेनुम्⟫ ... (⟪दुह्⟫ ⟪।⟫ ⟪रक्ष्⟫ ⟪।⟫ ⟪पा⟫)

⟪७⟫. ⟪द्विजदासो⟫ ⟪मृगमार्गेण⟫ ⟪ब्राह्मणग्रामम्⟫ ... (⟪गम्⟫ ⟪।⟫ ⟪इ⟫ ⟪।⟫ ⟪पद्⟫)

⟪८⟫. ⟪द्विजदासः⟫ ⟪शूद्रस्⟫ ... (⟪अस्⟫ ⟪२⟫ ⟪।⟫ ⟪भू⟫)

⟪९⟫. ⟪बालब्राह्मणी⟫ ... (⟪रुद्⟫ ⟪।⟫ ⟪आस्⟫ ⟪।⟫ ⟪मृ⟫)

⟪१०⟫. ⟪साधुजनो⟫ ⟪ऽधर्मम्⟫ ... (⟪द्विष्⟫ ⟪।⟫ ⟪न⟫ ⟪कृ⟫)

ب) در جملات تشکیل‌شده در بخش الف، فاعل و فعل را به جمع تبدیل کنید

## 17.8. تمرین ۲

افعال زیر را ترجمه کنید و ریشه مربوط به آن‌ها را مشخص نمایید:

⟪१⟫. ⟪अदन्ति⟫

⟪२⟫. ⟪सन्ति⟫

⟪३⟫. ⟪आसते⟫

⟪४⟫. ⟪यन्ति⟫

⟪५⟫. ⟪इच्छति⟫

⟪६⟫. ⟪कुर्वते⟫

⟪७⟫. ⟪गच्छन्ति⟫

⟪८⟫. ⟪जायते⟫

⟪९⟫. ⟪जयति⟫

⟪१०⟫. ⟪तनोति⟫

⟪११⟫. ⟪दहति⟫

⟪१२⟫. ⟪दोग्धि⟫

⟪१३⟫. ⟪पश्यति⟫

⟪१४⟫. ⟪द्विष्टे⟫

⟪१५⟫. ⟪नयन्ति⟫

⟪१६⟫. ⟪नृत्यति⟫

⟪१७⟫. ⟪पद्यन्ते⟫

⟪१८⟫. ⟪पिबति⟫

⟪१९⟫. ⟪पान्ति⟫

⟪२०⟫. ⟪पृच्छति⟫

⟪२१⟫. ⟪बुध्यन्ते⟫

⟪२२⟫. ⟪ब्रवीति⟫

⟪२३⟫. ⟪भवन्ति⟫

⟪२४⟫. ⟪मन्यते⟫

⟪२५⟫. ⟪मुञ्चन्ति⟫

⟪२६⟫. ⟪म्रियन्ते⟫

⟪२७⟫. ⟪यजते⟫

⟪२८⟫. ⟪युध्यन्ते⟫

⟪२९⟫. ⟪रक्षति⟫

⟪३०⟫. ⟪रोदिति⟫

⟪३१⟫. ⟪लभते⟫

⟪३२⟫. ⟪वक्ति⟫

⟪३३⟫. ⟪वदति⟫

⟪३४⟫. ⟪शृणोति⟫

⟪३५⟫. ⟪स्तौति⟫

⟪३६⟫. ⟪स्मरति⟫

⟪३७⟫. ⟪हन्ति⟫

⟪३८⟫. ⟪अश्नुवते⟫

⟪३९⟫. ⟪कुप्यते⟫

⟪४०⟫ ⟪कर्षन्ति⟫

⟪४१⟫. ⟪उद्यते⟫

⟪४२⟫. ⟪सहन्ते⟫

⟪४३⟫. ⟪सिच्यन्ते⟫

⟪४४⟫. ⟪आप्नोति⟫

⟪४५⟫. ⟪जीव्यते⟫

⟪४६⟫. ⟪दिश्यन्ते⟫