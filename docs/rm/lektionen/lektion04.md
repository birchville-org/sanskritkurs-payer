---
title: Lektion 4
subtitle: "Fragepronomina und Demonstrativpronomina im Nominativ"
lesson_id: 4
last_reconstructed: 2026-05-07
category: "Grammatik"
status: stable
---

# Lektion 4

::: deleteme-box
**En questa lecziun avais da aprender:**
- Pronoms interrogativs e demonstrativs en il nominativ
- Sandhi da -m final
- Sandhi da -e final
- Sandhi da -d final
- Frimas da dumonda simplas
:::
## 4.1. Dumondas da cumplement simpel (dumondas da paraula) e respostas

### 4.1.1. Pronomens interrogativs e pronomens demonstrativs

#### 4.1.1.1. Interrogativpronomen (praśnārthakasarvanāma n. = ⟪प्रश्नार्थकसर्वनामन्⟫)

::: grammar-box
| | Nominativ singular | Nominativ plural |
| :--- | :--- | :--- |
| **Neutrum** | **kim = ⟪किम्⟫** | **kāni = ⟪कानि⟫** |
| **Masculin** | **kas = ⟪कस्⟫** | **ke = ⟪के⟫** |
| **Feminin** | **kā = ⟪का⟫** | **kās = ⟪कास्⟫** |
:::
#### 4.1.1.2. Demonstrativpronomen (nidarśakasarvanāmāni n. = ⟪निदर्शकसर्वनामानि⟫)

::: grammar-box
| **Stivas** ||||
| :--- | :---: | :---: | :---: |
| | **tad = ⟪तद्⟫:br"er, sie, es; der, die, das":br(Erwähnte)** | **etad = ⟪एतद्⟫:br"dieser, diese, dieses":br(dem Sprechenden sehr Nahe)** | **idam = ⟪इदम्⟫:br"dieser, diese, dieses":br(Nahe)** |
| **Nominativ singular** ||||
| **Neutrum** | **tad = ⟪तद्⟫** | **etad = ⟪एतद्⟫** | **idam = ⟪इदम्⟫** |
| **Masculin** | **sa, so saḥ = ⟪स⟫ ⟪सो⟫ ⟪सः⟫** | **eṣa, eṣo, eṣaḥ = ⟪एष⟫ ⟪एषो⟫ ⟪एषः⟫** | **ayam = ⟪अयम्⟫** |
| **Feminin** | **sā = ⟪सा⟫** | **eṣā = ⟪एषा⟫** | **iyam = ⟪इयम्⟫** |
| **Nominativ plural** ||||
| **Neutrum** | **tāni = ⟪तानि⟫** | **etāni = ⟪एतानि⟫** | **imāni = ⟪इमानि⟫** |
| **Masculin** | **te = ⟪ते⟫** | **ete = ⟪एते⟫** | **ime = ⟪इमे⟫** |
| **Feminin** | **tās = ⟪तास्⟫** | **etās = ⟪एतास्⟫** | **imās ⟪इमास्⟫** |
:::
**Al Nom. sg. masc.:**
- **sa, eṣa** (⟪स⟫ ⟪एष⟫) stat avant tut las sons exceptà curt **-a**
- **so, eṣo** (⟪सो⟫ ⟪एषो⟫) stat avant **-a**, il qual vegn elidì cun Avagraha (absentà)
- **saḥ, eṣaḥ** (⟪सः⟫ ⟪एषः⟫) stat mo en pausa (fin da frasa, fin da vers)

Per pudair formar cun quellas pronomas (sarvanāman n.) frasis, è necessari la cognientscha da las suandantas reglas da Sandhi:

### 4.1.2. Sandhi von auslautendem -m

::: grammar-box
**La finala -m vegn sustituida avant consonantas cun Anusvāra (-ṃ). A la fin da la frasa u dal vers ed avant vocalas resta -m.**
:::
### 4.1.3. Sandhi da -m final

::: grammar-box
1. **vor a- bleibt -e unverändert: das anlautende a- elidiert mit Avagraha:**
   z.B. ete + agnayas » ete 'gnayaḥ = ⟪एते⟫ ⟪ऽग्नयः⟫ "Quests Fieu"

2. **Davart da auters vocals sco il curt a- vegn -e usitadamain sustitui cun -a cun hiatus. Era la sustituziun cun -ay vegn adina puspè:**
   per ex. ime + īśvarās » ima īśvarāḥ = ⟪इम⟫ ⟪ईश्वराः⟫ (rar: imay īśvarāh = ⟪इमयीश्वराः⟫) "QUESTS SIGNURS"
:::
### 4.1.4. Sandhi da -d final

::: grammar-box
**Final -d wird in Pausa sowie vor stimmlosen Gutturalen (k, kh) und Labialen (p, ph) durch -t ersetzt.**

La substituziun avant auts consonants vegn tractada pli tard. Evitai per il mument talas collisiuns da sons!
:::
### 4.1.5. Formulaziuns da dumonda

Mediant questas pronomas s'exprima per exempel las suaglias da dumonda:

::: no-header
| | | | |
| :--- | :--- | :--- | :--- |
| **Singular** | **Neutrum** | tat kim? /kiṃ tat?:br⟪तत्किम्⟫, ⟪किं⟫ ⟪तत्⟫:bretat kim? /kim etat?:br⟪एतत्किम्⟫, ⟪किमेतत्⟫:bridaṃ kim? / kim idam:br⟪इदं⟫ ⟪किम्⟫, ⟪किमिदम्⟫ | "Quai è quai?" |
| ^^ | **Masculin** | sa kaḥ? / kaḥ saḥ?:br⟪स⟫ ⟪कः⟫, ⟪कः⟫ ⟪सः⟫:breṣa kaḥ? / ka eṣaḥ:br⟪एष⟫ ⟪कः⟫, ⟪क⟫ ⟪एषः⟫:brayaṃ kaḥ? / ko 'yam?:br⟪अयं⟫ ⟪कः⟫, ⟪को⟫ ⟪ऽयम्⟫ | "Tgi è quai?":br"Quai è il?" |
| ^^ | **Feminin** | sā kā? / kā sā?:br⟪सा⟫ ⟪का⟫, ⟪का⟫ ⟪सा⟫:breṣā kā? / kaiṣā? (= kā + eṣā):br⟪एषा⟫ ⟪का⟫, ⟪कैषा⟫:briyaṃ kā? / keyam? (= kā + iyam):br⟪इयं⟫ ⟪का⟫, ⟪केयम्⟫ | "Tgi è quai?":br"Tgi è la?" |
| **Plural** | **Neutrum** | tāni kāni? / kāni tāni?:br⟪तानि⟫ ⟪कानि⟫, ⟪कानि⟫ ⟪तानि⟫:bretāni kāni? / kāny etāni?:br⟪एतानि⟫ ⟪कानि⟫, ⟪कान्येतानि⟫:brimāni kāni? / kānīmāni? (= kāni + imāni):br⟪इमानि⟫ ⟪कानि⟫, ⟪कानीमानि⟫ | |
| ^^ | **Masculin** | te ke? / ke te?:br⟪ते⟫ ⟪के⟫, ⟪के⟫ ⟪ते⟫:brete ke? / ka ete?:br⟪एते⟫ ⟪के⟫, ⟪क⟫ ⟪एते⟫:brime ke? / ka ime?:br⟪इमे⟫ ⟪के⟫, ⟪क⟫ ⟪इमे⟫ | |
| ^^ | **Feminin** | tāḥ kāḥ? / kās tāḥ?:br⟪ताः⟫ ⟪काः⟫, ⟪कास्ताः⟫:bretāḥ kāḥ? / kā etāḥ?:br⟪एताः⟫ ⟪काः⟫, ⟪का⟫ ⟪एताः⟫:brimāḥ kāḥ? / kā imāḥ?:br⟪इमाः⟫ ⟪काः⟫, ⟪का⟫ ⟪इमाः⟫ | |
:::
In exempli d'autras dumondas:

> viṣṇuḥ kaḥ? = ⟪विष्णुः⟫ ⟪कः⟫ Resposta: viṣṇur īśvaraḥ = ⟪विष्णुरीश्वरः⟫. (En ina frasa nominala po esser il subject per exempel al cumenzament per accentuar il collegi cun la dumonda.)

NOTA: Las respuondas che Vus pudais dar cun las cumpetenzas da Sanscrit fin qua a talas dumondas correspundan naturalmain anc betg en mintga relaziun ad in Sanscrit idiomaticamain bun.

## 4.2. Exercizi

**A) Formuladas dumondas oralbas basandà sin las suenterdidas cun il model viṣṇuḥ kaḥ (⟪विष्णुः⟩ ⟪कः⟩) e respundì las dumondas en sanscrit:**

śruti, śiva, brāhmaṇa, dvija (Plural), indrāṇī, dhenu, tulādhara, kālidāsa

= ⟪श्रुति⟫, ⟪शिव⟫, ⟪ब्राह्मण⟫, ⟪द्विज⟫ (⟪बहुवचनम्⟫), ⟪इन्द्राणी⟫, ⟪धेनु⟫, ⟪तुलाधर⟫, ⟪कालिदास⟫

**B) Formuladest questions per l'exercizi da lectura suenter il model etat kim (⟪एतत्किम्⟫) e respuondas las questions cun las paraulas specificadas ed ils pronom demonstrativ, per exempel eṣa bālaḥ (⟪एष⟫ ⟪बालः⟫):**

::: media
![](/images/lekt0401.webp)
(Imaggina: [Detagls](/rm/licenses#lekt0401))
:::
